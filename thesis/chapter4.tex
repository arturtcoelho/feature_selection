...
\textbf{Ridge never benefits from selection}: The linear model achieves its best RMSE at $k=1.00$ across all four datasets. Ridge regularization~\cite{hoerl1970ridge} already handles multicollinearity and noisy features effectively, making explicit feature removal unnecessary or harmful.
...
Figure~\ref{fig:stage1_cost} shows the distribution of computational costs across configurations. The cost patterns reveal important nuance: SHAP selection time varies dramatically by model. For Ridge, SHAP computation is near-instantaneous (linear SHAP is analytically tractable~\cite{lundberg2017unified}). For ExtraTrees, SHAP incurs substantial overhead, roughly 6-7x the cost of MI. For XGBoost, the TreeExplainer efficiency keeps SHAP competitive with other methods.
...
Table~\ref{tab:stage1_stability} summarizes Jaccard stability~\cite{Nogueira2018Stability} scores by strategy and retention level, averaged across datasets.
...
Table~\ref{tab:stage2_allstate} presents corresponding results for the Allstate dataset. On Allstate, patterns are similar to Superconductor:
